    ...
    RenderBuffer()
{
    ...
    _mesh = gameplay::Mesh::createMesh(gameplay::VertexFormat(elements, 3), initialVertexCount, true);
    _mesh->setVertexData(0, 0, initialVertexCount);

    bool largeIndices = (nbParticles * nbIndicesPerParticle) >= 65536;
    gameplay::Mesh::IndexFormat indexFormat = largeIndices ? gameplay::Mesh::INDEX32 : gameplay::Mesh::INDEX16;

    gameplay::MeshPart* meshPart = _mesh->addPart(gameplay::Mesh::TRIANGLES, indexFormat, initialIindexCount, true);
...
void SparkQuadRenderer::render(const Group& group,const DataSet* dataSet,RenderBuffer* renderBuffer) const
{
    ...
    if (_material)
    {
        gameplay::Technique* technique = _material->getTechnique();
        GP_ASSERT(technique);

        gameplay::MeshPart* part = buffer.getMesh()->getPart(0);
        unsigned int passCount = technique->getPassCount();

        for (unsigned int i=0; i<passCount; ++i)
        {
            gameplay::Pass* pass = technique->getPassByIndex(i);
            GP_ASSERT(pass);
